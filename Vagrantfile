...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  machines.each do |machine_settings|
    ...
    config.vm.define "#{machine_settings["hostname"]}", primary: machine_settings["primary"], autostart: machine_settings["autostart"] do |machine|
      ...
      machine.vm.provision :shell, :path => "provision.sh", :args => "deployment_configs #{config.ssh.username} #{machine_settings["hostname"]} #{machine_settings["hostname"]}.local #{machine_settings["basics"]} #{machine_settings["lamp"]} #{machine_settings["geoip"]} #{machine_settings["iptables"]} #{machine_settings["fail2ban"]}"
