  ...
  machines.each do |machine_settings|
    ...
    config.vm.define "#{machine_settings["hostname"]}", primary: machine_settings["primary"], autostart: machine_settings["autostart"] do |machine|
      ...
      if machine_settings["provision_script"].to_s.strip.length > 0
        machine.vm.provision :shell, :path => machine_settings["provision_script"], :args => "deployment_configs deployment_dbs #{config.ssh.username} #{machine_settings["hostname"]} #{machine_settings["hostname"]}.local #{machine_settings["basics"]} #{machine_settings["lamp"]} #{machine_settings["geoip"]} #{machine_settings["iptables"]} #{machine_settings["fail2ban"]}"
      end
