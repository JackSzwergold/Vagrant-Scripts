  ...
  machines.each do |machine_settings|
    ...
    config.vm.define "#{machine_settings["hostname"]}", primary: machine_settings["primary"], autostart: machine_settings["autostart"] do |machine|
      ...
      if machine_settings["provision_script"].to_s.strip.length > 0
        machine.vm.provision :shell, :privileged => true, :path => machine_settings["provision_script"], :args => "#{machine_settings["deployment_configs_path"]} #{machine_settings["deployment_dbs_path"]} #{config.ssh.username} #{machine_settings["machinename"]} #{machine_settings["hostname"]}.local #{machine_settings["basics"]} #{machine_settings["lamp"]} #{machine_settings["geoip"]} #{machine_settings["iptables"]} #{machine_settings["fail2ban"]}"
      end
      ...
      if machine_settings["provision_script_regular"].to_s.strip.length > 0
        machine.vm.provision :shell, :privileged => false, :path => machine_settings["provision_script_regular"], :args => "#{machine_settings["deployment_configs_path"]} #{machine_settings["deployment_dbs_path"]} #{config.ssh.username} #{machine_settings["machinename"]} #{machine_settings["hostname"]}.local #{machine_settings["basics"]} #{machine_settings["lamp"]} #{machine_settings["geoip"]} #{machine_settings["iptables"]} #{machine_settings["fail2ban"]}"
      end
